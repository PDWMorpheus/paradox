...
void World::SendGMText(int32 string_id, ...)
{
    ...
    for (SessionMap::iterator itr = m_sessions.begin(); itr != m_sessions.end(); ++itr)
    {
        ...
        if (itr->second->GetSecurity() < SEC_ADMINISTRATOR)
            continue;
...
bool World::AddDisable(int type, uint32 id, std::string comment)
{
    QueryResult result = WorldDatabase.PQuery("SELECT * FROM disables where sourcetype = %u and entry = %u", type, id);
    
    if (result)
        return false;

    WorldDatabase.PExecute("INSERT INTO disables (sourcetype, entry, flags, comment) VALUES (%u, %u, 1, '%s')", type, id, comment.c_str());
    return true;
}

bool World::RemoveDisable(int type, uint32 id)
{
    QueryResult result = WorldDatabase.PQuery("SELECT * FROM `disables` WHERE `sourcetype`='%u' and entry ='%u' and flags != 0", type, id);
    
    if (!result)
        return false;

    WorldDatabase.PExecute("DELETE FROM disables WHERE sourcetype = %u and entry = %u", type, id);
    return true;
}

void World::UpdateAccountsToVoter()
{
    QueryResult result = LoginDatabase.PQuery("SELECT id FROM account where vp > 223");
    do{
        QueryResult result = LoginDatabase.PQuery("SELECT id FROM account where vp > 223 LIMIT 1");
		Field *fields = result->Fetch();
        uint32 accId = fields[0].GetUInt32();
        LoginDatabase.PExecute("UPDATE account_access SET gmlevel=1 where id=%u", accId);
        LoginDatabase.PExecute("UPDATE account SET vp=0 where id=%u", accId);
    }while (result);
    return;
}