...
{
    Player* target = m_session->GetPlayer()->GetSelectedPlayer();
...
bool ChatHandler::HandleAddItemCommand(const char *args)
{
    ...
    if (!plTarget->CanUseID(DISABLE_TYPE_ITEM, itemId))
    {
	...
	if (!m_session->GetPlayer()->CanUseCommandOnPlayer(plTarget))
	{
		PSendSysMessage("You can't use commands on this player!");
		return true;
	}
...
bool ChatHandler::HandleAddItemSetCommand(const char *args)
{
	...
	if (!m_session->GetPlayer()->CanUseCommandOnPlayer(playerTarget))
	{
		PSendSysMessage("You can't use commands on this player!");
		return true;
	}
...
bool ChatHandler::HandleLookupCreatureDisplayCommand(const char *args)
{
    if (!*args)
        return false;
...
bool ChatHandler::HandleDieCommand(const char* /*args*/)
{
    ...
    {
		if (!m_session->GetPlayer()->CanUseCommandOnPlayer(target->ToPlayer()))
		{
			PSendSysMessage("You can't use commands on this player!");
			return true;
		}
...
bool ChatHandler::HandleReviveCommand(const char *args)
{
    ...
    if (target)
    {
        //Who doesn't want to be revived?
		if (!m_session->GetPlayer()->CanUseCommandOnPlayer(target))
		{
			PSendSysMessage("You can't use commands on this player!");
			return true;
...
bool ChatHandler::HandleAuraCommand(const char *args)
{
	...
	QueryResult result = WorldDatabase.PQuery("SELECT * FROM disables WHERE entry = %u AND sourcetype = 0 AND flags = 1", spellID);
	if (result)
	{
		SendSysMessage("This spell is disabled.");
		return true;
	}
    ...
    if (SpellEntry const *spellInfo = sSpellStore.LookupEntry(spellID))
	{
		...
		PSendSysMessage("Casting spell %u on target", spellID);
	}
    return true;
}
...
bool ChatHandler::HandleExploreCheatCommand(const char *args)
{
	...
	if (!m_session->GetPlayer()->CanUseCommandOnPlayer(chr))
	{
		PSendSysMessage("You can't use commands on this player!");
		return true;
	}
...
bool ChatHandler::HandleCharacterLevelCommand(const char *args)
{
    ...
    if (newlevel > 80)                         // After lvl 80, things start to get buggy
        newlevel = 80;
...
bool ChatHandler::HandlePossessCommand(const char * /*args*/)
{
	...
	{
		if (!m_session->GetPlayer()->CanUseCommandOnPlayer(pUnit->ToPlayer()))
		{
			PSendSysMessage("You can't use commands on this player!");
			return true;
		}
...
bool ChatHandler::HandleGodModeCheatCommand(const char* args)
{
	...
	if (argstr == "off")
	{
		m_session->GetPlayer()->m_cheatGod = false;
		SendSysMessage("Godmode is OFF. You can take damage.");
		return true;
	}
	else if (argstr == "on")
	{
		m_session->GetPlayer()->m_cheatGod = true;
...
bool ChatHandler::HandleCastTimeCheatCommand(const char* args)
{
	...
	if (argstr == "off")
	{
		m_session->GetPlayer()->m_cheatCastTime = false;
		SendSysMessage("CastTime cheat is OFF. Your spells will have a casttime.");
		return true;
	}
	else if (argstr == "on")
	{
		m_session->GetPlayer()->m_cheatCastTime = true;
...
bool ChatHandler::HandleCoolDownCheatCommand(const char* args)
{
	...
	if (argstr == "off")
	{
		m_session->GetPlayer()->m_cheatCoolDown = false;
		SendSysMessage("Cooldowncheat is OFF. You are on the global cooldown.");
		return true;
	}
	else if (argstr == "on")
	{
		m_session->GetPlayer()->m_cheatCoolDown = true;
		SendSysMessage("Cooldowncheat is ON. You have no global cooldown.");
		return true;
	}
	else
		return false;
...
bool ChatHandler::HandleFlyCheatCommand(const char* args)
{
	...
	if (argstr == "on")
	{
		m_session->GetPlayer()->m_cheatFly = true;
	...
	{
		m_session->GetPlayer()->m_cheatFly = false;
...
bool ChatHandler::HandlePowerCheatCommand(const char* args)
{
	...
	if (argstr == "off")
	{
		m_session->GetPlayer()->m_cheatPower = false;
		SendSysMessage("Powercheat is OFF. You need mana/rage/energy to use spells.");
		return true;
	}
	else if (argstr == "on")
	{
		m_session->GetPlayer()->m_cheatPower = true;
		SendSysMessage("Powercheat is ON. Don't need mana/rage/energy to use spells.");
		return true;
	}
	else
		return false;
...
bool ChatHandler::HandleWaterWalkCheatCommand(const char* args)
{
	...
	if (!*args)
	{
		if (m_session->GetPlayer()->GetCommandStatus(CHEAT_WATERWALK))
			argstr = "off";	
		else
			argstr = "on";
	}
	...
	if (argstr == "off")
	{
		m_session->GetPlayer()->m_cheatWaterWalk = false;
		m_session->GetPlayer()->SetMovement(MOVE_LAND_WALK);                // OFF
		SendSysMessage("Waterwalking is OFF. You can't walk on water.");
		return true;
	}
	else if (argstr == "on")
	{
		m_session->GetPlayer()->m_cheatWaterWalk = true;
		m_session->GetPlayer()->SetMovement(MOVE_WATER_WALK);               // ON
		SendSysMessage("Waterwalking is ON. You can walk on water.");
		return true;
	}
	else
		return false;
...
bool ChatHandler::HandleTriggerPassCheatCommand(const char* args)
{
	...
	if (!*args)
	{
		if (m_session->GetPlayer()->GetCommandStatus(CHEAT_TRIGGERPASS))
			argstr = "off";	
		else
			argstr = "on";
	}
	...
	if (argstr == "off")
	{
		m_session->GetPlayer()->m_cheatTriggerPass = false;
		SendSysMessage("Triggerpass is OFF. You need to be in a raidgroup to enter raids.");
		return true;
	}
	else if (argstr == "on")
	{
		m_session->GetPlayer()->m_cheatTriggerPass = true;
		SendSysMessage("Triggerpass is ON. You can enter a raid without a group.");
		return true;
	}
	else
		return false;
...
bool ChatHandler::HandleToggleModifyCommand(const char* args)
{
	if (!m_session && !m_session->GetPlayer())
		return false;
	...
	if (!*args)
	{
		if (m_session->GetPlayer()->GetCommandStatus(TOGGLE_MODIFY))
			argstr = "on";	
		else
			argstr = "off";
	}
	...
	if (argstr == "on")
	{
		m_session->GetPlayer()->m_toggleModify = false;
		PSendSysMessage(GetTrinityString(11002),"Modify","use commands on");
		return true;
	}
	else if (argstr == "off")
	{
		m_session->GetPlayer()->m_toggleModify = true;
		PSendSysMessage(GetTrinityString(11003),"Modify","use commands on");
		return true;
	}
	else
		return false;
...
bool ChatHandler::HandleToggleAppearCommand(const char* args)
{
	...
	if (!*args)
	{
		if (m_session->GetPlayer()->GetCommandStatus(TOGGLE_APPEAR))
			argstr = "on";	
		else
			argstr = "off";
	}
	...
	if (argstr == "on")
	{
		m_session->GetPlayer()->m_toggleAppear = false;
		PSendSysMessage(GetTrinityString(11002),"Appear","appear to");
		return true;
	}
	else if (argstr == "off")
	{
		m_session->GetPlayer()->m_toggleAppear = true;
		PSendSysMessage(GetTrinityString(11003),"Appear","appear to");
		return true;
	}
	else
		return false;
...
bool ChatHandler::HandleToggleSummonCommand(const char* args)
{
	...
	if (!*args)
	{
		if (m_session->GetPlayer()->GetCommandStatus(TOGGLE_SUMMON))
			argstr = "off";	
		else
			argstr = "on";
	}
	...
	if (argstr == "on")
	{
		m_session->GetPlayer()->m_toggleSummon = false;
		PSendSysMessage(GetTrinityString(11002),"Summon","summon");
		return true;
	}
	else if (argstr == "off")
	{
		m_session->GetPlayer()->m_toggleSummon = true;
		PSendSysMessage(GetTrinityString(11003),"Summon","summon");
		return true;
	}
	else
		return false;
...
bool ChatHandler::HandleDisableSpellCommand(const char* args)
{
    ...
    std::string idstr = cId;
    std::string comment = cComment;

    uint32 id = (uint32)atoi(cId);

    if (sWorld->AddDisable(DISABLE_TYPE_SPELL, id, comment))
        PSendSysMessage("Disabled spell %u for reason: '%s'", id, comment.c_str());
    else
    {
        PSendSysMessage("Spell: '%u' has already been disabled.", id);
        return true;
    }
...
bool ChatHandler::HandleDisableDisplayCommand(const char* args)
{
    if(!*args)
        return false;
    ...
    if (sWorld->AddDisable(DISABLE_TYPE_DISPLAYID, id, comment))
        PSendSysMessage("Disabled display ID %u for reason: '%s'", id, comment.c_str());
    else
    {
        PSendSysMessage("DisplayID: '%u' has already been disabled.", id);
        return true;
    }
...
bool ChatHandler::HandleDisableNPCCommand(const char* args)
{
    if(!*args)
        return false;
    ...
    if (sWorld->AddDisable(DISABLE_TYPE_NPC, id, comment))
        PSendSysMessage("Disabled NPC with entry %u for reason: '%s'", id, comment.c_str());
    else
    {
        PSendSysMessage("NPC: '%u' has already been disabled.", id);
        return true;
    }
...
bool ChatHandler::HandleDisableItemCommand(const char* args)
{
    if(!*args)
        return false;
    ...
    if (sWorld->AddDisable(DISABLE_TYPE_ITEM, id, comment))
        PSendSysMessage("Disabled item %u for reason: '%s'", id, comment.c_str());
    else
    {
        PSendSysMessage("Item: '%u' has already been disabled.", id);
        return true;
    }
...
bool ChatHandler::HandleDisableZoneCommand(const char* args)
{
    if(!*args)
        return false;
    ...
    if (sWorld->AddDisable(DISABLE_TYPE_ZONE, id, comment))
        PSendSysMessage("Disabled zone %u for reason: '%s'", id, comment.c_str());
    else
    {
        PSendSysMessage("Zone: '%u' has already been disabled.", id);
        return true;
    }
...
bool ChatHandler::HandleDisableObjectCommand(const char* args)
{
    if(!*args)
        return false;
    ...
    if (sWorld->AddDisable(DISABLE_TYPE_GOBJECT, id, comment))
        PSendSysMessage("Disabled object %u for reason: '%s'", id, comment.c_str());
    else
    {
        PSendSysMessage("Object: '%u' has already been disabled.", id);
        return true;
    }
...
/*bool ChatHandler::HandleServerDisableCommand(const char* args)
{
    if (!*args)
        return false;
	...
	sLog->outString("Made it this far!");
    ...
    {
		if (sWorld->AddDisable(DISABLE_TYPE_SPELL, id, comment))
			PSendSysMessage("Disabled %s %u for reason: '%s'", typestr.c_str(), id, comment.c_str());
		else
		{
			PSendSysMessage("Spell: '%u' has already been disabled.", id);
			return true;
    ...
    else if (typestr == "gobject")
    {
        if (sWorld->AddDisable(DISABLE_TYPE_GOBJECT, id, comment))
			PSendSysMessage("Disabled %s %u for reason: '%s'", typestr.c_str(), id, comment.c_str());
		else
		{
			PSendSysMessage("Gobject '%u' has already been disabled.", id);
			return true;
		}
	}
...
bool ChatHandler::HandleEnableSpellCommand(const char* args)
{
    if(!*args)
        return false;
    ...
    if (sWorld->RemoveDisable(DISABLE_TYPE_SPELL, id))
        PSendSysMessage("Enabled spell %u.", id);
    else
    {
        PSendSysMessage("Spell: '%u' is not disabled.", id);
        return true;
    }
...
bool ChatHandler::HandleEnableDisplayCommand(const char* args)
{
    if(!*args)
        return false;
    ...
    if (sWorld->RemoveDisable(DISABLE_TYPE_DISPLAYID, id))
        PSendSysMessage("Enabled display ID %u", id);
    else
    {
        PSendSysMessage("DisplayID: '%u' is not disabled.", id);
        return true;
    }
...
bool ChatHandler::HandleEnableNPCCommand(const char* args)
{
    if(!*args)
        return false;
    ...
    if (sWorld->RemoveDisable(DISABLE_TYPE_NPC, id))
        PSendSysMessage("Enabled NPC with entry %u", id);
    else
    {
        PSendSysMessage("NPC: '%u' is not disabled.", id);
        return true;
    }
...
bool ChatHandler::HandleEnableItemCommand(const char* args)
{
    if(!*args)
        return false;
    ...
    if (sWorld->RemoveDisable(DISABLE_TYPE_ITEM, id))
        PSendSysMessage("Enabled item %u", id);
    else
    {
        PSendSysMessage("Item: '%u' is not disabled.", id);
        return true;
    }
...
bool ChatHandler::HandleEnableZoneCommand(const char* args)
{
    if(!*args)
        return false;
    ...
    if (sWorld->RemoveDisable(DISABLE_TYPE_ZONE, id))
        PSendSysMessage("Enabled zone %u", id);
    else
    {
        PSendSysMessage("Zone: '%u' is not disabled.", id);
        return true;
    }
...
bool ChatHandler::HandleEnableObjectCommand(const char* args)
{
    if(!*args)
        return false;
    ...
    if (sWorld->RemoveDisable(DISABLE_TYPE_GOBJECT, id))
        PSendSysMessage("Enabled object %u", id);
    else
    {
        PSendSysMessage("Object: '%u' is not disabled.", id);
        return true;
    }
...
/*bool ChatHandler::HandleServerEnableCommand(const char* args)
{
    if (!*args)
        return false;
    ...
    {
		if (sWorld->RemoveDisable(DISABLE_TYPE_SPELL, id))
			PSendSysMessage("Enabled %s %u.",typestr.c_str(),id);
		else
		{
			PSendSysMessage("Spell: '%u' is not disabled.", id);
			return true;
...
bool ChatHandler::HandleMountCommand(const char* args)
{
	if (!*args)
	{
		PSendSysMessage("No model specified");
		return true;
	}
	...
	if (!m_session->GetPlayer()->CanUseID(DISABLE_TYPE_DISPLAYID, modelid))
    {
    	PSendSysMessage("You can't use this mount!");
    	return true;
    }

	if (!target)
	{
		PSendSysMessage("No target specified");
		return true;
	}
		
	if (target->IsMounted())
	{
		PSendSysMessage("Target is mounted");
		return true;
	}
		
	target->SetUInt32Value(UNIT_FIELD_MOUNTDISPLAYID, modelid);
    PSendSysMessage("Mounted target with displayId: %u", modelid);
    return true;
}

bool ChatHandler::TestColors(const char* args)
{
    PSendSysMessage("|cffff6060 LIGHTRED (1)");
    PSendSysMessage("|cff00ccff LIGHTBLUE (2)");
    PSendSysMessage("|cff00C78C TORQUISEBLUE (3)");
    PSendSysMessage("|cff00FF7F SPRINGGREEN (4)");
    PSendSysMessage("|cffADFF2F GREENYELLOW (5)");
    //PSendSysMessage("|cff0000ff BLUE"); Illegible
    PSendSysMessage("|cffDA70D6 PURPLE (6)");
    PSendSysMessage("|cff00ff00 GREEN (7)");
    PSendSysMessage("|cffff0000 RED (8)");
    PSendSysMessage("|cffffcc00 GOLD (9)");
    PSendSysMessage("|cffFFC125 GOLD2 (10)");
    PSendSysMessage("|cff888888 GREY (11)");
    PSendSysMessage("|cffffffff WHITE (12)");
    PSendSysMessage("|cffbbbbbb SUBWHITE (13)");
    PSendSysMessage("|cffff00ff MAGENTA (14)");
    PSendSysMessage("|cffffff00 YELLOW (15)");
    PSendSysMessage("|cffFF4500 ORANGEY (16)");
    PSendSysMessage("|cffCD661D CHOCOLATE (17)");
    PSendSysMessage("|cff00ffff CYAN (18)");
    //PSendSysMessage("|cff8B8B83 IVORY"); Illegible
    PSendSysMessage("|cffFFFFE0 LIGHTYELLOW (19)");
    PSendSysMessage("|cff71C671 SEXGREEN (20)");
    PSendSysMessage("|cff388E8E SEXTEAL (21)");
    PSendSysMessage("|cffC67171 SEXPINK (22)");
    PSendSysMessage("|cff00E5EE SEXBLUE (23)");
    PSendSysMessage("|cffFF6EB4 SEXHOTPINK (24)");
    return true;
}

bool ChatHandler::HandleModifyAnnounceColorCommand(const char* args)
{
    if (!*args)
    ...
    switch (color)
    {
        ...
        case 24:
            ...
            colorstr1 = "SexyHotPink";
            break;
        default:
            return false;
            break;
    }
    PSendSysMessage("Chatcolor successfully set to %s.", colorstr1.c_str());
    return true;
}
