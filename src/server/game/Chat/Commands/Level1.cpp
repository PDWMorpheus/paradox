...
bool ChatHandler::HandleSummonCommand(const char* args)
{
    ...
    if (!target)
    {
        SendSysMessage("Player does not exist or is offline.");
        return true;
    }

    if (target->GetCommandStatus(TOGGLE_SUMMON)  && !m_session->GetPlayer()->IsAdmin())
    {
        SendSysMessage("This player has summon toggled off, you cannot summon them.");
		return true;
    }
...
bool ChatHandler::HandleAppearCommand(const char* args)
{
    ...
    if (!target)
    {
        SendSysMessage("Player doesn't exist or is offline.");
        return true;
    }

    if (target->GetCommandStatus(TOGGLE_APPEAR) && !m_session->GetPlayer()->IsAdmin())
    {
        SendSysMessage("This player has appear toggled off, you cannot appear to them.");
		return true;
    }
