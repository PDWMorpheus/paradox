...
public:
    ...
    ChatCommand* GetCommands() const
    {
        ...
        static ChatCommand gobjectCommandTable[] =
        {
            ...
            { "select",         SEC_GAMEMASTER,     false, &HandleGameObjectTemplateCommand,      "", NULL },
            { "move",           SEC_GAMEMASTER,     false, &HandleGameObjectMoveCommand,      "", NULL },
            { "near",           SEC_GAMEMASTER,     false, &HandleGameObjectNearCommand,      "", NULL },
            //{ "info",           SEC_GAMEMASTER,     false, &HandleGameObjectTargetCommand,    "", NULL },
            { "rotate",         SEC_GAMEMASTER,     false, &HandleGameObjectTurnCommand,      "", NULL },
            { "spawn",          SEC_GAMEMASTER,     false, &HandleGameObjectAddCommand,       "", NULL },
    ...
    static bool HandleGameObjectActivateCommand(ChatHandler* handler, const char* args)
    {
			...
			uint32 selectedGo = handler->GetSession()->GetPlayer()->GetSelectedGobject();
			if (!selectedGo || selectedGo < 1)
			{
				handler->SendSysMessage("No gameobject selected!");
				return true;
			}
...
*/
    	...
    	{
    		handler->PSendSysMessage("You can't spawn this gameobject!");
    		return true;
    	}
    ...
    static bool HandleGameObjectTargetCommand(ChatHandler* handler, const char* args)
    {
        ...
        handler->GetSession()->GetPlayer()->SetSelectedGobject(lowguid);
    ...
    static bool HandleGameObjectTemplateCommand(ChatHandler* handler, const char* args)
    {
        uint32 entry = 0;
        std::string name;
        uint32 guid = 0;
