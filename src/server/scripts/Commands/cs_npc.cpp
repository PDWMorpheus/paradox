...
public:
	...
	static bool HandleDetermineNpcSpawn(ChatHandler* handler, const char* args)
    {
		...
		{
			HandleNpcAddTempSpawnCommand(handler, idstr);
			return true;
		}
		return true;
	}
    ...
    static bool HandleNpcAddCommand(ChatHandler* handler, const char* args)
    {
    	...
    	{
    		handler->PSendSysMessage("You can't spawn this NPC!");
    		return true;
    	}
		...
		std::string cName = pCreature->GetName();

        map->Add(pCreature);
        sObjectMgr->AddCreatureToGrid(db_guid, sObjectMgr->GetCreatureData(db_guid));
		handler->PSendSysMessage("Spawned NPC '%u' (%s)", id, cName.c_str());
        return true;
    }
    ...
    static bool HandleNpcDeleteCommand(ChatHandler* handler, const char* args)
    {
        ...
        if (*args)
        {
            ...
            if (WorldDatabase.PQuery("SELECT * FROM creature where guid=%u and id>=500000 and id<=500144",lowguid) && !handler->GetSession()->GetPlayer()->IsAdmin())
          {
            handler->PSendSysMessage("Cannot delete mall vendors.");
            return false;
           }
    ...
    static bool HandleNpcMoveCommand(ChatHandler* handler, const char* args)
    {
        ...
        if (WorldDatabase.PQuery("SELECT * FROM creature where guid=%u and id>=500000 and id<=500144",lowguid) && !handler->GetSession()->GetPlayer()->IsAdmin())
          {
            handler->PSendSysMessage("Cannot move mall vendors.");
            return false;
           }
    ...
    */
    ...
    static bool HandleNpcAddTempSpawnCommand(ChatHandler* handler, const char* args)
    {
        ...
        if (!handler->GetSession()->GetPlayer()->CanUseID(DISABLE_TYPE_NPC, id))
    	{
    		handler->SendSysMessage("You can't spawn this NPC!");
    		return true;
    	}

		QueryResult result = WorldDatabase.PQuery("SELECT name FROM creature_template WHERE entry = %u", id);
		if (result)
		{
			...
			handler->PSendSysMessage("Spawned NPC '%u' (%s)",id,name.c_str());
			return true;
		}

		chr->SummonCreature(id, *chr, TEMPSUMMON_CORPSE_DESPAWN, 120);
		handler->PSendSysMessage("Spawned NPC '%u'", id);
        return true;
    }
