...
public:
	...
	if (!handler->GetSession()->GetPlayer()->CanUseCommandOnPlayer(targetPlayer))
	{
	...
	// Disable learning disabled spells.
	if(sDisableMgr->IsDisabledFor(DISABLE_TYPE_SPELL, spell, targetPlayer))
	{

	   handler->PSendSysMessage("You cannot learn disabled spells.");
	   handler->SetSentErrorMessage(true);
	   return false;
    ...
    static bool HandleLearnAllGMCommand(ChatHandler* handler, const char* /*args*/)
    {
        for (uint32 i = 0; i < GetSpellStore()->GetNumRows(); ++i)
        {
            ...
            if(sDisableMgr->IsDisabledFor(DISABLE_TYPE_SPELL, spellInfo->Id, handler->GetSession()->GetPlayer()))
                continue;
