...
public:
    ...
    static bool HandleAccountAddonCommand(ChatHandler* handler, const char* args)
    {
        ...
        {
            handler->SendSysMessage("Invalid expansion id!");
            return true;
        }

        // No SQL injection
        LoginDatabase.PExecute("UPDATE account SET expansion = '%d' WHERE id = '%u'", expansion, account_id);
        handler->PSendSysMessage("Successfully set account expansion to %u.", expansion);
        return true;
    }
    ...
    static bool HandleAccountCreateCommand(ChatHandler* handler, const char* args)
    {
        ...
        {
        case AOR_OK:
            handler->PSendSysMessage("Successfully created account '%s'", account_name.c_str());
            break;
        case AOR_NAME_TOO_LONG:
            handler->SendSysMessage("Specified account name is too long.");
            return true;;
        case AOR_NAME_ALREDY_EXIST:
            handler->SendSysMessage("Specified account already exists!");
            return true;
        case AOR_DB_INTERNAL_ERROR:
            handler->SendSysMessage("Internal database error. SQL format was probably updated");
            return true;
        default:
            handler->PSendSysMessage("Account '%s' was not created.", account_name.c_str());
            return true;
        }
    ...
    static bool HandleAccountDeleteCommand(ChatHandler* handler, const char* args)
    {
        ...
        {
            handler->SendSysMessage("Your target is a higher ranked GM!");
            return true;
        }
        ...
        {
        case AOR_OK:
            handler->PSendSysMessage("Account %s successfully deleted.", account_name.c_str());
            break;
        case AOR_NAME_NOT_EXIST:
            handler->PSendSysMessage("Account %s doesn't exist!", account_name.c_str());
            return true;
        case AOR_DB_INTERNAL_ERROR:
            handler->SendSysMessage("Internal database error. SQL format was probably updated");
            return true;
        default:
            handler->PSendSysMessage("Account '%s' was not deleted.", account_name.c_str());
            return true;
        }
    ...
    static bool HandleAccountSetAddonCommand(ChatHandler* handler, const char *args)
    {
        ...
        if (expansion < 0 || uint8(expansion) > sWorld->getIntConfig(CONFIG_EXPANSION))
        {
            handler->SendSysMessage("Invalid expansion id specified.");
            return true;
        }
    ...
    static bool HandleAccountSetGmLevelCommand(ChatHandler* handler, const char *args)
    {
        ...
        if (isAccountNameGiven)
        {
            ...
            {
                handler->PSendSysMessage("Account %s doesn't exist!", targetAccountName.c_str());
                return true;
            }
        }
        ...
        if (gm > SEC_CONSOLE)
        {
            handler->SendSysMessage("Incorrect gm level.");
            return true;
        }
        ...
        handler->PSendSysMessage("Account '%s' GM level has been updated to '%s'. This change requires a client restart.", targetAccountName.c_str(), gm);
        return true;
    }
    ...
    static bool HandleAccountSetPasswordCommand(ChatHandler* handler, const char *args)
    {
        ...
        {
            handler->SendSysMessage("The specified passwords don't match.");
            return true;
        }
        ...
        {
        case AOR_OK:
            handler->SendSysMessage("Successfully changed password.");
            break;
        case AOR_NAME_NOT_EXIST:
            handler->PSendSysMessage("Account %s doesn't exist.", account_name.c_str());
            return true;
        case AOR_PASS_TOO_LONG:
            handler->SendSysMessage("The specified password is too long.");
            return true;
        default:
            handler->SendSysMessage("Unable to change password.");
            return true;
        }
        return true;
    }
